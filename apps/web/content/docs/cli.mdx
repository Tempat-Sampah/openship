...
Run the whole platform locally — bundled API + embedded database, plus the dashboard (lazy-downloaded on first run). Local access needs no login (loopback).

```bash
openship up                       # API on :4000, dashboard on :3001
openship up --port 4000 --dashboard-port 3001
openship up --no-ui               # API only, no dashboard
openship up --data-dir ~/.openship/data
```
